...
mod tests {
    ...
    fn each_adr_has_three_or_more_sections() {
        for path in &list_adr_files() {
            let content = read_adr(path);
            let filename = path.file_name().unwrap().to_string_lossy();
            let section_count = content.lines()
    ...
    fn risk_entry_has_field(entry: &str, field: &str) -> bool {
        entry.lines().any(|l| l.trim().starts_with(&format!("| {}", field)))
    }
    ...
    fn risk_register_is_large_enough() {
        ...
        let line_count = content.lines().count();
        assert!(line_count >= 100, "Risk register too short: {line_count} lines, expected >= 100");
    }

    #[test]
    fn risk_register_has_title() {
        let content = risk_register_content();
        assert!(content.starts_with("# "), "Must start with a level-1 heading");
        assert!(content.contains("Risk Register"), "Title must contain 'Risk Register'");
    }

    #[test]
    fn risk_register_has_all_risks() {
        let content = risk_register_content();
        let expected_ids = ["R-001","R-002","R-003","R-004","R-005",
    ...
    fn each_risk_has_required_fields() {
        ...
        let required_fields = ["Category", "Probability", "Impact", "Severity", "Status", "Owner"];
    ...
    fn each_risk_has_description_and_mitigation() {
        ...
        for entry in &entries {
            ...
            assert!(
                ...
                "Risk {}: missing Description section", id_line
            );
            assert!(
                entry.contains("**Mitigation**"),
                "Risk {}: missing Mitigation section", id_line
            );
            let has_contingency = entry.contains("**Contingency**");
            assert!(
                has_contingency,
                "Risk {}: missing Contingency section", id_line
            );
        }
    }
    ...
    fn each_risk_severity_matches_probability_times_impact() {
        ...
        for entry in &entries {
            ...
            let severity_num: u32 = severity_str.split_whitespace().next()
    ...
    fn risk_status_summary_counts_match() {
        ...
        let mut status_counts: std::collections::HashMap<String, u32> =
            [("Open","Open"), ("Mitigated","Mitigated"), ("Closed","Closed")]
                .iter().map(|(k, _)| (k.to_string(), 0u32)).collect();
        ...
        if let Some(start) = summary_section_start {
            ...
            for (status, count) in &status_counts {
                ...
                let found = summary_section.lines()
    ...
    fn acceptance_has_deliverables_section() {
        ...
        assert!(c.contains("Final Deliverables"), "Missing deliverables section");
        let table_lines: Vec<&str> = c.lines()
            .filter(|l| l.trim().starts_with('|') && l.contains('|'))
            .collect();
        assert!(table_lines.len() >= 8, "Expected 8+ deliverable rows, got {}", table_lines.len());
    }

    #[test]
    fn acceptance_has_equivalence_table() {
        let c = acceptance_content();
        assert!(c.contains("Functional Equivalence Checklist"), "Missing equivalence table");
        let equivalence_rows: Vec<&str> = c.lines()
            .filter(|l| l.trim().starts_with('|') && l.contains("ga-"))
            .collect();
        assert!(equivalence_rows.len() >= 16, "Expected 16+ equivalence items, got {}", equivalence_rows.len());
    }

    #[test]
    fn acceptance_has_pass_fail_criteria() {
        let c = acceptance_content();
        assert!(c.contains("Pass/Fail Criteria"), "Missing Pass/Fail Criteria section");
    }

    #[test]
    fn acceptance_has_build_metrics() {
        let c = acceptance_content();
        assert!(c.contains("Current Build Metrics"), "Missing Build Metrics section");
        assert!(c.contains("Release binary size"), "Missing binary size metric");
        assert!(c.contains("Workspace test count"), "Missing test count metric");
    }

    #[test]
    fn acceptance_has_testing_regimen() {
        let c = acceptance_content();
        assert!(c.contains("Testing Regimen"), "Missing Testing Regimen section");
        assert!(c.contains("cargo test --workspace"), "Missing test command");
    }

    #[test]
    fn ci_workflow_exists() {
        let ci_path = project_root().join(".github").join("workflows").join("ci.yml");
        ...
        assert!(ci_content.contains("cargo build --release"), "CI missing release build");
        assert!(ci_content.contains("cargo clippy"), "CI missing clippy step");
    }

    #[test]
    fn ci_has_required_jobs() {
        let ci_content = read_file(&project_root().join(".github").join("workflows").join("ci.yml"));
    ...
    fn acceptance_notes_binary_size_target() {
        ...
        assert!(has_target, "Acceptance criteria should reference the 15MB binary size target");
    }
    ...
    fn acceptance_deliverables_use_status_tags() {
        ...
        assert!(total >= 6, "Expected 6+ deliverable status tags, got {total}");
    }
    ...
    fn equivalence_table_has_status_column() {
        ...
        assert!(done_count >= 14, "Expected 14+ ✅ Done items, got {done_count}");
    }
    ...
    fn docs_directory_structure() {
        ...
        assert!(dir.join("risk-register.md").exists(), "Missing docs/risk-register.md");
        assert!(dir.join("acceptance-criteria.md").exists(), "Missing docs/acceptance-criteria.md");
    }
}
