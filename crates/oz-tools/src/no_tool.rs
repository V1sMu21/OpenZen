...
impl ToolHandler for NoTool {
    ...
    fn description(&self) -> String {
        "Provide a final text response to the user. CALLING THIS TOOL IS A COMMITMENT THAT THE TASK IS COMPLETE: every checklist item is completed and every [verify] acceptance assertion in task_spec.md passes. If not there yet, keep executing — do not respond. Only call this when the original task is actually finished.".to_string()
    }
...
mod tests {
    ...
    async fn test_execute_with_response() {
        let r = NoTool
            .execute(serde_json::json!({"response": "hello"}), &ctx())
            .await
            .unwrap();
