...
impl WebExecuteJsTool {
    ...
    pub fn set_browser(&self, client: BrowserClient) {
        *self.browser.lock().unwrap() = Some(client);
    }
}
...
impl ToolHandler for WebExecuteJsTool {
    ...
    fn description(&self) -> String {
        "Execute JavaScript in a connected browser tab. Use to click elements, \
         ...
         Multi-call OK. Use web_list_tabs to find tab IDs."
            .into()
    }
    ...
    async fn execute(
        ...
        if code.is_empty() {
            return Ok(ToolOutput::bad_json("web_execute_js: code is required"));
        }
        ...
        Ok(ToolOutput::success(
            serde_json::json!({"status": "ok", "result": truncated}),
        ))
    }
}

pub struct WebListTabsTool {
    browser: Mutex<Option<BrowserClient>>,
}
...
impl ToolHandler for WebListTabsTool {
    fn name(&self) -> String {
        "web_list_tabs".to_string()
    }
    fn description(&self) -> String {
        "List connected browser tabs with IDs and URLs.".into()
    }
    fn parameters(&self) -> serde_json::Value {
    ...
    async fn execute(
        ...
        let list: Vec<serde_json::Value> = tabs
            ...
            .collect();
        Ok(ToolOutput::success(
            serde_json::json!({"tabs": list, "count": list.len()}),
        ))
    }
}
